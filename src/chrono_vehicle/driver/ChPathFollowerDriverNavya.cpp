...
ChPathFollowerDriverNavya::ChPathFollowerDriverNavya(ChVehicle& vehicle,
...
void ChPathFollowerDriverNavya::Create() {
...
void ChPathFollowerDriverNavya::Reset() {
    m_steeringPID.Reset(m_vehicle);
    m_speedPID.Reset(m_vehicle);
}

void ChPathFollowerDriverNavya::Advance(double step) {
...
void ChPathFollowerDriverNavya::ExportPathPovray(const std::string& out_dir) {
...
ChPathFollowerDriverNavyaXT::ChPathFollowerDriverNavyaXT(ChVehicle& vehicle,
                                           std::shared_ptr<ChBezierCurve> path,
                                           const std::string& path_name,
                                           double target_speed,
                                           bool isClosedPath,
...
void ChPathFollowerDriverNavyaXT::Create() {
    // Reset the steering and speed controllers
...
void ChPathFollowerDriverNavyaXT::Reset() {
    m_steeringXT.Reset(m_vehicle);
    m_speedPID.Reset(m_vehicle);
}

void ChPathFollowerDriverNavyaXT::Advance(double step) {
    // Set the throttle and braking values based on the output from the speed controller.
    double out_speed = m_speedPID.Advance(m_vehicle, m_target_speed, step);
    ChClampValue(out_speed, -1.0, 1.0);
...
void ChPathFollowerDriverNavyaXT::ExportPathPovray(const std::string& out_dir) {
