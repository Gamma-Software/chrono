...
#include "chrono_models/vehicle/trailor/Trailor_RigidTire.h"
...
/// @addtogroup vehicle_models_Trailor
/// @{
...
/// Note that this is an absTrailor class which cannot be instantiated.  Instead, use one of the
/// concrete classes HMMWV_Full or HMMWV_Reduced.
class CH_MODELS_API Trailor {
  public:
    virtual ~Trailor();
    ...
    ChVehicle& GetVehicle() const { return *m_vehicle; }
  ...
  protected:
    ...
    Trailor();
    Trailor(ChSystem* system);

    virtual Trailor_Vehicle* CreateVehicle() = 0;
...
class CH_MODELS_API TrailorModel : public Trailor{
public:
    TrailorModel() {}
    TrailorModel(ChSystem* system) : Trailor(system) {}

private:
    virtual Trailor_Vehicle* CreateVehicle() override;
};
