...
// =============================================================================
//
// Rear Trailor suspension subsystems (simple leafspring work a like).
//
...
// ---------------------------------------------------------------------------------------
// Trailor spring functor class - implements a linear spring + bump stop + rebound stop
// ---------------------------------------------------------------------------------------
class Trailor_SpringForceRear : public ChLinkSpringCB::ForceFunctor {
  public:
    Trailor_SpringForceRear(double spring_constant, double min_length, double max_length);
...
Trailor_SpringForceRear::Trailor_SpringForceRear(double spring_constant, double min_length, double max_length) :
...
double Trailor_SpringForceRear::operator()(double time, double rest_length, double length, double vel, ChLinkSpringCB* link) {
    /*
     *
    */
...
// -----------------------------------------------------------------------------
// Trailor shock functor class - implements a nonlinear damper
// -----------------------------------------------------------------------------
class Trailor_ShockForceRear : public ChLinkSpringCB::ForceFunctor {
  public:
    Trailor_ShockForceRear(double compression_slope,
...
Trailor_ShockForceRear::Trailor_ShockForceRear(double compression_slope,
                                   double compression_degressivity,
                                   double expansion_slope,
                                   double expansion_degressivity)
...
double Trailor_ShockForceRear::operator()(double time, double rest_length, double length, double vel, ChLinkSpringCB* link) {
    /*
...
Trailor_LeafspringAxle::Trailor_LeafspringAxle(const std::string& name,
...
*/
    m_springForceCB = new Trailor_SpringForceRear(m_data.springCoefficient,
        m_data.springMinLength,
        m_data.springMaxLength);

    m_shockForceCB = new Trailor_ShockForceRear(m_data.damperCoefficient,
...
// Destructors
// -----------------------------------------------------------------------------
Trailor_LeafspringAxle::~Trailor_LeafspringAxle() {
    delete m_springForceCB;
    delete m_shockForceCB;
}

const ChVector<> Trailor_LeafspringAxle::getLocation(PointId which) {
    switch (which) {
        case SPRING_A:
